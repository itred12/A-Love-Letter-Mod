...
class ParchmentToBookSingleBindingRecipeOld(id: ResourceLocation, recipeCategory: CraftingBookCategory, bindingIngredients: NonNullList<Ingredient>) : CustomRecipe(id, recipeCategory) {
    ...
    companion object {
        ...
        // Referenced mostly from the shapeless recipe serializer
        class Serializer : RecipeSerializer<ParchmentToBookSingleBindingRecipeOld> {

            private val NAME = ALoveLetter.modLoc("crafting_parchment_binding_singleitem")

            override fun fromJson(
                pRecipeId: ResourceLocation,
                pSerializedRecipe: JsonObject
            ): ParchmentToBookSingleBindingRecipeOld {

                // Obtain the category from the Json field
                val categoryField = GsonHelper.getAsString(pSerializedRecipe, "group", "")
                val category = CraftingBookCategory.CODEC.byName(categoryField)!!

                val ingredients = ALLRecipeUtils.itemsFromJson(GsonHelper.getAsJsonArray(pSerializedRecipe, "binding_ingredients"))
                if (ingredients.isEmpty()) {
                    throw JsonParseException("No ingredients for crafting_parchment recipe")
                } else {
                    return ParchmentToBookSingleBindingRecipeOld(pRecipeId, category, ingredients)
                }
            ...
            override fun fromNetwork(
                ...
                val ingredientArraySize = pBuffer.readVarInt()
                val ingredients = NonNullList.withSize(ingredientArraySize, Ingredient.EMPTY)

                for (index in 0..<ingredientArraySize) {
            ...
            override fun toNetwork(
                pBuffer: FriendlyByteBuf,
                pRecipe: ParchmentToBookSingleBindingRecipeOld
            ) {
